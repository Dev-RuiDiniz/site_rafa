...
export default function VisualEditorPage({ params }: { params: Promise<{ pageId: string }> }) {
  ...
  const getPublicPath = useCallback((slug: string, isSystem: boolean | undefined) => {
    if (!slug) return "/";
    if (slug === "home") return "/";
  ...
  const publicPath = getPublicPath(page.slug, page.isSystem);
