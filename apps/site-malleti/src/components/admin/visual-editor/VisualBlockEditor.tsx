...
const RichTextEditor = dynamic(() => import("@/components/admin/RichTextEditor"), {
  ssr: false,
  loading: () => (
    <div className="h-[220px] border border-dashed border-gray-300 dark:border-gray-600 rounded-lg flex items-center justify-center text-xs text-gray-500">
      Carregando editor...
    </div>
  ),
});
...
function LayoutControls({ layout, onChange }: {
  ...
  const currentLayout: LayoutSettings = {
    paddingTop: layout?.paddingTop ?? 80,
    paddingBottom: layout?.paddingBottom ?? 80,
    maxWidth: layout?.maxWidth ?? "boxed",
    contentAlign: layout?.contentAlign ?? "center",
  ...
  const handleChange = (field: keyof LayoutSettings, value: string | number | undefined) => {
    onChange({ ...currentLayout, [field]: value });
  };

  return (
    <div className="border border-dashed border-gray-300 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50 mt-4">
      <button
        type="button"
      ...
      >
        <span>Ajustes avançados de layout</span>
        <span className="text-[10px] uppercase tracking-wide text-gray-500">{expanded ? "Ocultar" : "Expandir"}</span>
      </button>
      {expanded && (
        <div className="px-3 pb-3 pt-1 space-y-3 text-xs">
          <div className="grid grid-cols-2 gap-2">
            <InputField
              ...
              value={String(currentLayout.paddingTop ?? 0)}
              onChange={(v) => handleChange("paddingTop", parseInt(v) || 0)}
              type="number"
            />
            <InputField
              label="Padding Inferior (px)"
              value={String(currentLayout.paddingBottom ?? 0)}
              onChange={(v) => handleChange("paddingBottom", parseInt(v) || 0)}
              type="number"
            />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <SelectField
              ...
              value={currentLayout.maxWidth ?? "boxed"}
              onChange={(v) => handleChange("maxWidth", v)}
              options={[
                { value: "boxed", label: "Boxed (1280px)" },
            ...
            <SelectField
              ...
              value={currentLayout.verticalAlign ?? "center"}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <ColorField
              label="Cor do Texto"
              value={currentLayout.textColor || "#111111"}
              onChange={(value) => handleChange("textColor", value)}
            />
          </div>
          <TextareaField
            label="Gradiente / CSS de fundo"
            value={currentLayout.backgroundGradient || ""}
            onChange={(v) => handleChange("backgroundGradient", v)}
            rows={2}
            placeholder="linear-gradient(135deg, #000, #444)"
          />
          <ImageUploader
            label="Imagem de Fundo da Seção"
            value={currentLayout.backgroundImage || ""}
            onChange={(url) => handleChange("backgroundImage", url)}
          />
          <div className="grid grid-cols-2 gap-2">
            <InputField
              label="ID Âncora (opcional)"
              value={currentLayout.anchorId || ""}
              onChange={(v) => handleChange("anchorId", v)}
              placeholder="ex: sessao-servicos"
            />
            <InputField
              label="Classe CSS extra"
              value={currentLayout.customClass || ""}
              onChange={(v) => handleChange("customClass", v)}
              placeholder="ex: bg-overlay"
            />
          </div>
          <p className="text-[11px] text-gray-500">
            Esses valores são armazenados dentro de <code>content.layout</code> e podem ser lidos no front-end para personalizar cada bloco.
          </p>
        </div>
      )}
    </div>
...
function RichTextField({ label, value, onChange, placeholder, helperText }: {
  label: string;
  value: string;
  onChange: (value: string) => void;
  placeholder?: string;
  ...
  return (
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800/60">
      <div className="flex items-start justify-between gap-3 px-3 py-2">
        <div>
          <p className="text-xs font-medium text-gray-700 dark:text-gray-200">{label}</p>
          <p className="text-[11px] text-gray-500 dark:text-gray-400">
            {helperText || "Sincroniza com o campo de conteúdo simples acima."}
          </p>
        </div>
        <button
          type="button"
        ...
        >
          {expanded ? "Ocultar" : "Usar editor rico"}
        </button>
      </div>
      {expanded && (
...
function InputField({
  ...
  return (
    <div className="space-y-2">
      ...
      {enableRich && (
        <RichTextField
          label={richLabel || `${label} (avançado)`}
          value={value}
          onChange={onChange}
          placeholder={richPlaceholder || placeholder}
          helperText={
            richHelperText || "Use o editor rico para aplicar tipografia customizada, cores e alinhamentos."
          }
        />
      )}
    </div>
    ...
    <div className="space-y-2">
      ...
      {enableRich && (
        <RichTextField
          ...
          placeholder={richPlaceholder || placeholder}
          helperText={
            richHelperText || "Use o editor rico para aplicar formatação, imagens, listas e links."
          }
        />
      )}
    </div>
...
function HeroEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      />
      <RichTextField
        label="Descrição com formatação"
        value={(content.description as string) || ""}
        onChange={(v) => onChange({ ...content, description: v })}
        placeholder="Use títulos, listas, imagens e links"
        helperText="Opcional — substitui o texto acima quando preenchido."
      />
      <div className="grid grid-cols-2 gap-2">
        <SelectField
          label="Alinhamento"
      ...
      <LayoutControls layout={layout} onChange={handleLayoutChange} />
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      ...
      <RichTextField
        ...
        value={(content.description as string) || ""}
        onChange={(v) => onChange({ ...content, description: v })}
        placeholder="Explique os diferenciais da seção com liberdade"
      />
...
function TextEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      />
      <RichTextField
        label="Conteúdo avançado"
        value={(content.content as string) || ""}
        onChange={(v) => onChange({ ...content, content: v })}
        placeholder="Digite o conteúdo com formatação avançada"
        helperText="Mantém o texto acima sincronizado; use quando precisar de formatação rica."
      />
      <div className="grid grid-cols-2 gap-2">
        <SelectField
          label="Alinhamento"
  ...
  return (
    <div className="space-y-3">
      ...
      <RichTextField
        ...
        value={(content.description as string) || ""}
        onChange={(v) => onChange({ ...content, description: v })}
        placeholder="Permite listas, links e destaques"
      />
      <div className="grid grid-cols-2 gap-2">
        <InputField
          label="Botão - Texto"
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        ...
        <button
          onClick={(e) => { e.stopPropagation(); onChange({ ...content, cards: [...cards, { image: "", title: "Novo card", description: "", link: "" }] }); }}
          className="w-full py-1.5 text-xs border border-dashed rounded hover:bg-gray-50 dark:hover:bg-gray-700"
        >
          + Adicionar Card
        </button>
  ...
  return (
    <div className="space-y-3">
      <InputField
        ...
        enableRich
        richLabel="Badge com formatação"
      />
      <InputField
        label="Título"
        value={(content.title as string) || ""}
        ...
        enableRich
        richLabel="Título com formatação"
        richHelperText="Use o editor para alterar tipografia, cor, tamanhos e alinhamentos do título."
      />
      <TextareaField
        label="Descrição"
        value={(content.description as string) || ""}
        ...
        rows={3}
        enableRich
        richLabel="Descrição com formatação"
        richPlaceholder="Insira títulos, listas, imagens e links para enriquecer o texto"
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      ...
      <RichTextField
        ...
        value={(content.description as string) || ""}
        onChange={(v) => onChange({ ...content, description: v })}
        placeholder="Detalhe os benefícios com liberdade"
      />
      ...
      {faqs.map((faq, index) => (
        <div key={index} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg space-y-2">
          ...
          <TextareaField
            ...
            rows={3}
            enableRich
            richLabel="Resposta com formatação"
          />
        </div>
      ))}
      <button
  ...
  return (
    <div className="space-y-3">
      ...
      <TextareaField
        ...
        rows={4}
        enableRich
        richLabel="Citação com formatação"
      />
  ...
  return (
    <div className="space-y-3">
      ...
      <TextareaField
        ...
        onChange={(v) => onChange({ ...content, description1: v })}
        rows={3}
        enableRich
      />
      <TextareaField
        ...
        onChange={(v) => onChange({ ...content, description2: v })}
        rows={3}
        enableRich
      />
      <InputField label="Texto do Botão" value={(content.buttonText as string) || ""} onChange={(v) => onChange({ ...content, buttonText: v })} />
