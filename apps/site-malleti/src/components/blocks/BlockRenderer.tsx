...
type RichTextTag = keyof Pick<JSX.IntrinsicElements, "div" | "p" | "span" | "h1" | "h2" | "h3" | "h4">;
...
function RichTextContent({
  ...
  const contentValue = (value && value.trim().length > 0 ? value : undefined) ?? fallback;
  if (!contentValue) return null;
  const html = /<[a-z][\s\S]*>/i.test(contentValue) ? contentValue : contentValue.replace(/\n/g, "<br />");
  const Tag = as as RichTextTag;
...
function getLayoutProps(layout: LayoutSettings | undefined, defaults: { paddingTop: number; paddingBottom: number }): SectionLayoutProps {
  if (!layout) {
    return {
      sectionStyle: {},
      wrapperStyle: {},
  ...
  const sectionStyle: CSSProperties = {
    paddingTop: `${layout.paddingTop ?? defaults.paddingTop}px`,
    paddingBottom: `${layout.paddingBottom ?? defaults.paddingBottom}px`,
  };

  if (layout.backgroundColor) {
    sectionStyle.backgroundColor = layout.backgroundColor;
  }

  if (layout.backgroundGradient && layout.backgroundImage) {
    sectionStyle.backgroundImage = `${layout.backgroundGradient}, url(${layout.backgroundImage})`;
  } else if (layout.backgroundGradient) {
    sectionStyle.backgroundImage = layout.backgroundGradient;
  } else if (layout.backgroundImage) {
    sectionStyle.backgroundImage = `url(${layout.backgroundImage})`;
  }
  ...
  if (layout.textColor) {
    sectionStyle.color = layout.textColor;
  }

  const wrapperStyle: CSSProperties = {};

  if (layout.maxWidth && layout.maxWidth !== "full") {
    wrapperStyle.maxWidth = layout.maxWidth === "boxed" ? "1280px" : "1440px";
    wrapperStyle.marginLeft = "auto";
    wrapperStyle.marginRight = "auto";
  }

  if (layout.contentAlign === "left") {
  ...
  return {
    anchorId: layout.anchorId?.trim() || undefined,
    customClass: layout.customClass,
    sectionStyle,
    wrapperStyle,
  };
}
...
function HeroBlock({ content }: { content: Record<string, unknown> }) {
  ...
  const layout = content.layout as LayoutSettings | undefined;
  const { anchorId, customClass, sectionStyle, wrapperStyle } = getLayoutProps(layout, { paddingTop: 160, paddingBottom: 160 });
  const overlayValue = clamp(overlay / 100, 0, 1);
  ...
  const heroStyle: CSSProperties = {
    ...sectionStyle,
    ["--hero-overlay" as keyof CSSProperties]: overlayValue.toString(),
  };

  return (
    <section
      id={anchorId}
      className={`relative min-h-screen flex items-center justify-center overflow-hidden bg-black hero-overlay ${customClass || ""}`.trim()}
      style={heroStyle}
    >
      ...
      <div className={`relative z-10 container mx-auto px-6 lg:px-12 text-white flex flex-col ${alignClass} py-32`} style={wrapperStyle}>
        <motion.div
        ...
        >
          <RichTextContent
            ...
            className="inline-block px-4 py-1 text-xs uppercase tracking-[0.2em] border border-white/30 text-white/80 mb-6"
            as="span"
          />
...
function TextBlock({ content }: { content: Record<string, unknown> }) {
  ...
  const layout = content.layout as LayoutSettings | undefined;
  const { anchorId, customClass, sectionStyle, wrapperStyle } = getLayoutProps(layout, { paddingTop: 96, paddingBottom: 96 });
  ...
  return (
    <section
      id={anchorId}
      className={`py-24 ${bgClass} ${customClass || ""}`.trim()}
      style={sectionStyle}
    >
      <div className="container mx-auto px-6 lg:px-12" style={wrapperStyle}>
        <motion.div
          initial={{ opacity: 0, y: 30 }}
        ...
        >
          <RichTextContent
            value={content.subtitle as string}
            className={`text-sm uppercase tracking-[0.2em] ${background === "black" ? "text-gray-400" : "text-gray-500"} mb-4 block`}
            as="span"
          />
...
function GalleryBlock({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <section id={anchorId} className={`py-24 bg-white ${customClass || ""}`.trim()} style={sectionStyle}>
      <div className="container mx-auto px-6 lg:px-12" style={wrapperStyle}>
        {(content.title as string) && (
          ...
          >
            <RichTextContent value={content.subtitle as string} className="text-sm uppercase tracking-[0.2em] text-gray-500 mb-4 block" as="span" />
            <RichTextContent value={content.title as string} className="text-4xl md:text-5xl font-serif font-semibold text-black" as="h2" />
          </motion.div>
        )}
...
function VideoBlock({ content }: { content: Record<string, unknown> }) {
  ...
  const videoTitle = stripHtml((content.title as string) || "Vídeo destacado");
...
function CTABlock({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <section
      ...
      style={sectionStyle}
    >
      <div className="container mx-auto px-6 lg:px-12 text-center" style={wrapperStyle}>
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
...
function CardsBlock({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <section
      id={anchorId}
      className={`py-24 bg-gray-50 ${customClass || ""}`.trim()}
      style={sectionStyle}
    >
      <div className="container mx-auto px-6 lg:px-12" style={wrapperStyle}>
        {(content.title as string) && (
      ...
      <div className="relative z-10 container mx-auto px-6 lg:px-12 h-full flex items-center">
        <div className="max-w-3xl">
          <AnimatePresence mode="wait">
            ...
            >
              <RichTextContent value={currentBanner.subtitle} className="text-white/60 text-lg md:text-xl font-light tracking-wide mb-2" as="h2" />
              <RichTextContent
                value={currentBanner.title}
                className="text-5xl md:text-7xl lg:text-8xl font-serif font-semibold text-white tracking-tight mb-6"
                as="h1"
              />
              <RichTextContent
    ...
    <section ref={ref} className="py-24 lg:py-32 bg-white overflow-hidden">
      <div className="container mx-auto px-6 lg:px-12">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
            <RichTextContent
              value={content.subtitle as string}
              fallback="Coleção"
              className="text-sm uppercase tracking-[0.2em] text-gray-500 mb-4 block"
              as="span"
            />
            <RichTextContent
              value={content.title as string}
              fallback="Produtos em Destaque"
              className="text-3xl md:text-4xl font-serif font-semibold text-black"
              as="h2"
            />
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }} className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-12 h-12 flex items-center justify-center border border-black/20 hover:bg-black hover:text-white hover:border-black transition-all duration-300"
              >
              ...
              <button
                ...
                className="w-12 h-12 flex items-center justify-center border border-black/20 hover:bg-black hover:text-white hover:border-black transition-all duração-300"
              >
        ...
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 0.6, delay: 0.3 }} className="relative">
          <div ref={carouselRef} className="flex gap-6 overflow-x-auto no-scrollbar pb-4 snap-x snap-mandatory pr-6 lg:pr-12">
...
function WhyChooseUsBlock({ content }: { content: Record<string, unknown> }) {
  ...
  const layout = content.layout as LayoutSettings | undefined;
  const { anchorId, customClass, sectionStyle, wrapperStyle } = getLayoutProps(layout, { paddingTop: 96, paddingBottom: 128 });

  return (
    <section
      ref={ref}
      id={anchorId}
      className={`py-24 lg:py-32 bg-gray-50 ${customClass || ""}`.trim()}
      style={sectionStyle}
    >
      <div className="container mx-auto px-6 lg:px-12" style={wrapperStyle}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }} className="flex flex-col justify-center">
            <RichTextContent
              value={content.subtitle as string}
              fallback="Por que nos escolher"
              className="text-sm uppercase tracking-[0.2em] text-gray-500 mb-4"
              as="span"
            />
            <RichTextContent
              value={content.title as string}
              fallback="Excelência em cada detalhe"
              className="text-4xl md:text-5xl font-serif font-semibold text-black mb-6 leading-tight"
              as="h2"
            />
            <RichTextContent value={content.description as string} className="text-gray-600 text-lg leading-relaxed mb-8" as="p" />
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }}>
            ...
            <div className="space-y-3 mb-10">
              {features.map((item, index) => (
                <motion.div key={index} initial={{ opacity: 0, x: 20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.4, delay: 0.4 + index * 0.1 }}>
                  <BulletList items={[item]} className="text-sm text-white/90" />
                </motion.div>
              ))}
            </div>
...
function ContactHeroBlock({ content }: { content: Record<string, unknown> }) {
  const layout = content.layout as LayoutSettings | undefined;
  const { anchorId, customClass, sectionStyle, wrapperStyle } = getLayoutProps(layout, { paddingTop: 128, paddingBottom: 96 });
  ...
  return (
    <section id={anchorId} className={`bg-white ${customClass || ""}`.trim()} style={sectionStyle}>
      <div className="container mx-auto px-6 lg:px-12" style={wrapperStyle}>
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          />
          <RichTextContent
            value={(content.title as string) || "Fale Conosco"}
            className="text-5xl md:text-6xl lg:text-7xl font-serif font-semibold text-black mb-6 leading-tight"
          />
          <RichTextContent value={content.description as string} className="text-gray-600 text-lg leading-relaxed" />
        </motion.div>
      </div>
    </section>
  );
          ...
          {options.map((option, index) => (
            ...
            >
              ...
              <div>
                ...
                <RichTextContent value={option.description} className="text-gray-500 text-sm" />
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <div className="bg-black text-white p-10 lg:p-12 h-full">
      ...
      {(content.hours as string) && (
        <div className="mt-12 pt-8 border-t border-white/10">
          ...
          <RichTextContent value={content.hours as string} className="space-y-2 text-gray-400 text-sm" />
        </div>
      )}
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            ...
            <RichTextContent value={content.description as string} className="text-gray-300 text-lg leading-relaxed mb-8" />
            {(content.buttonText as string) && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="bg-gray-50 p-8 hover:shadow-lg transition-shadow group">
              ...
              <RichTextContent value={service.description} className="text-gray-600 mb-6 leading-relaxed" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {benefits.map((benefit, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="bg-white p-6 border border-gray-100">
                ...
                <RichTextContent value={benefit.description} className="text-gray-500 text-sm" />
              </motion.div>
            ))}
          </div>
        </div>
    ...
    <section className="py-24 bg-black text-white">
      <div className="container mx-auto px-6 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <RichTextContent value={content.description as string} className="text-gray-400 max-w-2xl mx-auto mb-8" />
          {(content.buttonText as string) && (
            <Button size="lg" className="bg-white text-black hover:bg-gray-100" asChild>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {faqs.map((faq, index) => (
            <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="border-b border-gray-100 pb-6">
              ...
              <RichTextContent value={faq.answer} className="text-gray-600 leading-relaxed" />
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            ...
            <RichTextContent
              value={(content.description as string) || "Trabalhamos com as marcas mais prestigiadas do mercado mundial de mobiliário para salões de beleza e spas."}
          ...
          {brands.map((brand, index) => (
            <motion.div key={brand.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.2 }} className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center`}>
              ...
              <div className={index % 2 === 1 ? "lg:order-1" : ""}>
                ...
                <BulletList items={brand.highlights} className="space-y-3 mb-8 text-gray-700" />
                <Button variant="outline" className="border-black text-black hover:bg-black hover:text-white transition-all duration-300 group" asChild>
                  <Link href={`/marcas/${brand.slug}`}>
                    Conhecer marca {brand.name}<HiArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" /></Link>
                </Button>
              </div>
            </motion.div>
          ))}
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="max-w-5xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <RichTextContent
              value={(content.description as string) || "A SHR é o elo entre as maiores marcas internacionais e o mercado brasileiro."}
    ...
    <section className="py-24 bg-black text-white">
      <div className="container mx-auto px-6 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <RichTextContent
            value={(content.description as string) || "Explore nosso catálogo completo e descubra como as marcas que representamos podem transformar seu salão."}
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
            ...
            <RichTextContent
              value={(content.description as string) || "Há mais de uma década, a SHR é referência no mercado brasileiro de mobiliário para salões de beleza e spas."}
      ...
      <div className="container mx-auto px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <RichTextContent
              value={(content.description1 as string) || "A Maletti é uma das mais prestigiadas fabricantes de mobiliário para salões de beleza do mundo. Com mais de 55 anos de história, a marca italiana é sinônimo de inovação, qualidade e design sofisticado."}
              className="text-gray-600 text-lg leading-relaxed mb-6"
            />
            <RichTextContent
              value={(content.description2 as string) || "Como distribuidor exclusivo no Brasil, a SHR oferece toda a linha de produtos Maletti com garantia de originalidade, suporte técnico especializado e peças de reposição originais."}
    ...
    <section className="py-24 bg-gray-50">
      <div className="container mx-auto px-6 lg:px-12 text-center">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <RichTextContent
            value={(content.description as string) || "Entre em contato conosco e descubra como os produtos Maletti podem elevar o padrão do seu negócio."}
